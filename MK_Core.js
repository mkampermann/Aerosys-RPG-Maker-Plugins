/*:
 * @target MZ
 * @plugindesc [Tier 0] [Version 1.4.2] (please read the License Part 1)
 * @author Aerosys
...
MK.thirdPartySupport.beforeMapLoaded = function() {
    ...
    // Altimit Movement
    if (MK.isPluginEnabled('AltimitMovement')) {
    ...
    // Shora Lighting
    if (typeof $shoraLayer !== 'undefined') {
        $shoraLayer.reset();
    }
}
...
MK.isPluginEnabled = function(pluginName) {
    return $plugins.filter(Boolean).some(plugin => plugin.status && plugin.name == pluginName);
}
...
// Alias: Call beforeMapLoaded & afterMapLoaded
...
Scene_Map.prototype.onMapLoaded = function() {
    ...
    MK.isMapRefreshRequired = false;
}

MK.requestMapRefresh = function() {
    MK.isMapRefreshRequired = true;
}
...
MK.loadChildMaps = function(mapId) {
    ...
    for (let mapInfo of additionalMapIds) {
        DataManager.loadAdditionalMap(mapInfo.name.toLowerCase(), mapInfo.id);
    }
}
...
DataManager.onXhrLoadAdditionalMap = function(name, xhr) {
    if (xhr.status < 400) {
        const data = JSON.parse(xhr.responseText);
        this.onLoadAdditionalMap(name, data);
    }
}

DataManager.onLoadAdditionalMap = function(name, object) {
    this.onLoad(object);
    MK.additionalMaps[name] = object;
    MK.n_mapsLoading -= 1;
}

DataManager.loadAdditionalMap = function(name, mapId) {
    MK.additionalMaps[name] = undefined;
	MK.n_mapsLoading++;
	...
	xhr.onload = () => this.onXhrLoadAdditionalMap(name, xhr);
...
DataManager.isMapLoaded = function() {
	return MK.alias.isMapLoaded.call(this) && MK.n_mapsLoading == 0;
};
...
if ('MV' == Utils.RPGMAKER_NAME) {
    ...
    DataManager.onLoad = function(object) {
        ...
        if (this.isMapObject(object)) {
            ...
            object.events.filter(Boolean).forEach(this.extractMetadata);
        }
    }
}
...
MK.getMapFromDisk = function(mapId = $gameMap.mapId()) {
    if (MK.maps[mapId])
...
MK.eraseRegionId = function(dataMap, regionId) {
    MK.eraseRegionIds(dataMap, [regionId]);
}

MK.eraseRegionIds = function(dataMap, ids) {
    for (let x = 0; x < dataMap.width; x++) {
        for (let y = 0; y < dataMap.height; y++) {
            if (ids.contains(MK.regionId(x, y, dataMap)))
...
Tilemap.isShadowingTileStack = function(tileStack) {
    return tileStack.slice(0, 4).some(tileId => Tilemap.isShadowingTile(tileId) || Tilemap.isWaterfallTile(tileId));
}

MK.automaticallyPutShadows = function(dataMap, horizontal = 3, vertical1 = 3, vertical2 = 1) {
    MK.requireNonNull(dataMap, "MK.automaticallyPutShadows", "dataMap");

    const shadowMatrix = MK.createMatrix(dataMap.width * 2, dataMap.height * 2);
    const isShadowing = (x, y) => Tilemap.isShadowingTileStack(MK.getTileStack(x, y, dataMap));
    const isTop = (x, y) => MK.getTileStack(x, y, dataMap).slice(0, 4).some(tileId => Tilemap.isTopTile(tileId));
    const isWall = (x, y) => MK.getTileStack(x, y, dataMap).slice(0, 4).some(tileId => Tilemap.isWallSideTile(tileId));

    // draw shadow
    for (let x = 0; x < 2 * dataMap.width; x++) {
        for (let y = 0; y < 2 * dataMap.height; y++) {
            if (isShadowing(Math.floor(x / 2), Math.floor(y / 2))) {

                const d = Math.ceil(vertical1 / 2);

                if (!isShadowing(Math.floor(x / 2), Math.floor(y / 2) - d)) {
                    // is Top
                    for (let i = 0; i < horizontal + 1; i++) {
                        const x2 = x + i;
                        const y2 = y + vertical1;
                        if (MK.isValid(x2, y2, shadowMatrix) && !isTop(Math.floor(x2 / 2), Math.floor(y2 / 2)))
                            shadowMatrix[x2][y2] = true;
                    }
                } else {
                    // is not Top
                    for (let i = 0; i < horizontal + 1; i++) {
                        for (let j = 0; j < vertical2 + 1; j++) {
                            const x2 = x + i;
                            const y2 = y + j;
                            if (MK.isValid(x2, y2, shadowMatrix) && !isTop(Math.floor(x2 / 2), Math.floor(y2 / 2)))
                                shadowMatrix[x2][y2] = true;
            ...
            if (isWall(Math.floor(x / 2), Math.floor(y / 2)) && vertical2 > 0)
                shadowMatrix[x][y] = true;
        }
    }
    if (vertical2 == 0) {
        for (let x = 0; x < 2 * dataMap.width; x++) {
    ...
    for (let x = 0; x < dataMap.width; x++) {
        for (let y = 0; y < dataMap.height; y++) {
            ...
            if (shadowMatrix[2 * x][2 * y])         shadowId |= 1;
            if (shadowMatrix[2 * x + 1][2 * y])     shadowId |= 2;
            if (shadowMatrix[2 * x][2 * y + 1])     shadowId |= 4;
            if (shadowMatrix[2 * x + 1][2 * y + 1]) shadowId |= 8;

            MK.setShadowId(x, y, dataMap, shadowId);
        }
    }
}